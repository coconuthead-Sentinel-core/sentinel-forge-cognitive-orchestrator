...
Write-Host " [STEP 1] LATTICE COMPRESSION (Artifact Cleanup)..." -ForegroundColor Yellow
$artifacts = @(
...
Write-Host " [STEP 2] PROTOCOL ENCAPSULATION (Bushido Framework)..." -ForegroundColor Yellow
...
if (Test-Path "meta_context/A1.Ω.Master_Optimization.json") {
...
# --- 3. Identity Locking ---
Write-Host " [STEP 3] IDENTITY LOCKING (Crystalline Navigator)..." -ForegroundColor Yellow
$manifestPath = "meta_context/A1.Ω.Master_Optimization.json"
...
# --- 4. Credential Warmup ---
Write-Host " [STEP 4] CREDENTIAL WARMUP (Managed Identity)..." -ForegroundColor Yellow
if (Test-Path ".env") {
