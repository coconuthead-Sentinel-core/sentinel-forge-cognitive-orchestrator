...
Protects all information assets of Sovereign Forge — source code, credentials, voice data, Sentinel Profile, and Azure infrastructure.
...
| Voice data — session only, no recordings | 100% of sessions |
...
|-------|---------------|
| Azure OpenAI API Key | 🔴 Confidential |
| Azure Endpoint URL | 🔴 Confidential |
| `.env` file | 🔴 Confidential |
| Sentinel Profile (JSONStore) | 🟡 Internal |
| Source code | 🟡 Internal |
| GitHub repository (public) | 🟢 Public |
| SDLC + compliance docs | 🟢 Public |
...
- STT processing occurs in browser (Web Speech API) — audio does not leave the device
- AI text response sent to Azure OpenAI — covered by Microsoft DPA
- TTS playback is local — no external audio service
...
- `.env`: local machine only — excluded from GitHub via `.gitignore`
...
1. Detect → Log in `INCIDENT_LOG.md`
2. Contain → Set `MOCK_AI=true` if needed
