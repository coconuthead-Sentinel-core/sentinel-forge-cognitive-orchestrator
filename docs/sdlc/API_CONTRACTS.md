...
```json
{
    "message": "string (required) — user input text or glyph sequence",
    "lens": "adhd | autism | dyslexia | neurotypical (optional — defaults to profile)"
}
```
...
```json
{
    "response": "string — AI-processed cognitive output",
    "lens_applied": "adhd",
...
**Purpose:** Full snapshot — zones, mirrors, CNO-AX performance, lens usage.
...
**Purpose:** Real-time combined event stream — cognitive events + periodic metrics.
...
Azure OpenAI failures handled internally — adapter falls back to mock silently.
