...
def test_transform_simple_sentence(lens):
    ...
    assert "💡 processes" in result # Key concept
    assert "⚙️ system" in result # Key concept
...
def test_transform_longer_sentence(lens):
    ...
    assert "[ 🧠 Central Idea ]" in result
    assert "The quick brown fox" in result # Central idea
    assert "💡 brown" in result
    assert "⚙️ eats" in result
...
def test_transform_multiple_sentences(lens):
    ...
    assert "Second sentence provides details." not in result # Should not be in the central idea
    
    # Key concepts should be from the other sentences
    assert "💡 adds" in result
    assert "⚙️ details" in result
...
def test_no_key_concepts_found(lens):
    ...
    text = "A." # No words with 4+ letters
    result = lens.transform_context(text)

    assert "[ 🧠 Central Idea ]" in result
    assert "A." in result
    # Should not have concept branches
